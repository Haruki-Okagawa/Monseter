...
namespace Shinobu_am_me
{
    public class Break_Mesh
    {
       ...
       private static Mesh_Child[] victim_child = new Mesh_Child[8];
        ...
        //縦と横の真ん中の切り分ける座標
        private static float center_side = 0;
        private static float center_length = 0;
        private static float center_depth = 0;
        ...
        public static GameObject[] Cut(GameObject victim)
        {
            ...
            center_depth = (front_point + back_point) / 2.0f;
            center_length = (up_point + down_point) / 2.0f;
            center_side = (right_point + left_point) / 2.0f;
            ...
            for (int sub = 0; sub < victim_mesh.subMeshCount; sub++)
            {
                ...
                for (int i = 0; i < indices.Length; i += 3)
                {
                    ...
                    for (int t = 0; t < pos.Length; t++)
                    {
                        ...
                        if (temp.z < center_depth)
                        {
                            if (temp.y > center_length)
                            {
                                if (temp.x < center_side)
                                {
                                    pos[t] = 0;
                                }
                                else
            ...
            //分割回数を入
            Space_Cut_Cube temp_cut = cube_obj[0].GetComponent<Space_Cut_Cube>();
            ...
            for (int i = 1; i < one_eight_mesh.Length; i++)
            {
                ...
                //temp_rigid.AddForce(temp_velocity/8);
                cube_obj[i].AddComponent(typeof(Space_Cut_Cube));

                temp_cut = cube_obj[i].GetComponent<Space_Cut_Cube>();
                temp_cut.Cut_Count = cutcount;
        ...
        ////それぞれの法線を作成　0正面,1右面,2背面,3左面,4上面,5下面
        //Vector3[] normals_6 = {
            ...
            for (int i = 0; i < victim_child.Length; i++)
            {
                ...
                victim_child[i].AddTriangle(
                    new Vector3[]
                    {
                            xyz_vertics_set[0,0],
                            xyz_vertics_set[0,1],
                            new Vector3(center_side,center_length,center_depth)
                    },
                    new Vector3[]
                    {
                        hole_normal[0],
        ...
        private static void Create_normal(Vector3 point1, Vector3 point2,int victim_num,ref Vector3[,] xyz_vertics,ref Vector3[] xyz_normal)
        {
            ...
            //xラインの法線
            if (System.Math.Abs(point1.x - center_side) < float.Epsilon && System.Math.Abs(point2.x - center_side) < float.Epsilon)
            {
                if (victim_num % 2 == 0)
                {
                    hole_normal = new Vector3(1.0f, 0, 0);
            ...
            //yラインの法線
            else if (System.Math.Abs(point1.y - center_length) < float.Epsilon && System.Math.Abs(point2.y - center_length) < float.Epsilon)
            {
            ...
            //zラインの法線
            else if (System.Math.Abs(point1.z - center_depth) < float.Epsilon && System.Math.Abs(point2.z - center_depth) < float.Epsilon)
            {
                //Debug.Log("スカーレット・シュート！");
